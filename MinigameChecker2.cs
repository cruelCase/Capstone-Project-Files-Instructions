...
public class MinigameChecker2 : MonoBehaviour
{
    ...
    private ProfilePlayerData GetActiveUserProfile()
    {
        ...
        {
            Debug.LogError("MinigameChecker2: No active user found. Cannot load main quiz scene.");
            ShowBlockedPanel();
            return null;
        }
        ...
        if (!File.Exists(path))
        {
            Debug.LogError($"MinigameChecker2: Profile file not found for user '{activeUser}'. Cannot load main quiz scene.");
            ShowBlockedPanel();
            return null;
        }
        ...
        if (profile == null)
        {
            Debug.LogError("MinigameChecker2: Failed to parse profile JSON. Cannot load main quiz scene.");
            ShowBlockedPanel();
            return null;
        }
    ...
    {
        if (profile.miniQuiz1cCompleted && profile.miniQuiz2cCompleted && profile.miniQuiz3cCompleted)
    ...
    private void ShowBlockedPanel()
    {
        ...
        {
            Debug.LogWarning("MinigameChecker2: blockedPanel is not assigned in the inspector.");
            return;
        }
